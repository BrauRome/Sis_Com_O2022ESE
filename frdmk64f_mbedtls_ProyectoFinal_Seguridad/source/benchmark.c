...
/* NXP: Move buffer to NON-CACHED memory because of HW accel */ 
    ...
    unsigned char other_buf[BUFSIZE];
#endif /* DCACHE */
...
int main( int argc, char *argv[] )
{
    ...
    unsigned char signature_output[71];
    unsigned char verification_output[200];
    unsigned char signature_ver_output[200];
    ...
    memset( hash_output, 0xDD, sizeof( hash_output ) );
    memset( signature_output, 0xEE, sizeof( signature_output ) );
    memset( verification_output, 0xFF, sizeof( verification_output ) );
    memset( signature_ver_output, 0xA0, sizeof( signature_ver_output ) );
    ...
    if ( todo.chacha20 )
    {
    	...
    	PRINTF("\r\nEncrypting...\r\n");
		mbedtls_chacha20_crypt( buf, buf, 0U, BUFSIZE, buf, other_buf );
		PRINTF("\r\nEncrypted Message:");
		uint32_t * my_buf =(uint32_t *) other_buf;
		for(uint32_t i=0;i<256;i++){
			PRINTF("%08x,",*my_buf++);
		}
		PRINTF("\r\n");

		//mbedtls_chacha20_crypt( buf, buf, 0U, BUFSIZE, other_buf, buf );
	//	PRINTF("\r\nDecryption:");
		//my_buf =(uint32_t *) buf;
		//for(uint32_t i=0;i<256;i++){
			//PRINTF("%08x,",*my_buf++);
    ...
    PRINTF("\r\nDigesting...\r\n");
    if( todo.md4 )
    	mbedtls_md4_ret( other_buf, BUFSIZE, hash_output);

    PRINTF("\r\nMessage Digest:");
    char * my_buf2 =(char *) hash_output;
    for(uint32_t i=0;i<sizeof(hash_output);i++){
    	PRINTF("%02x,",*my_buf2++);
    }
    PRINTF("\r\n");
    ...
    if( todo.ecdsa )
    {
        ...
        int return_val = 0;
        char error_msg[200];
        char * ptr_error_msg = &error_msg[0];

        memset( buf, 0x2A, sizeof( buf ) );

        mbedtls_ecdsa_init( &ecdsa );

		if( mbedtls_ecdsa_genkey( &ecdsa, MBEDTLS_ECP_DP_SECP256R1, myrand, NULL ) != 0 )
			mbedtls_exit( 1 );
		ecp_clear_precomputed( &ecdsa.grp );
		PRINTF("\r\nSigning Message Digest...\r\n");
		return_val = mbedtls_ecdsa_write_signature( &ecdsa, MBEDTLS_MD_MD4, hash_output, sizeof(hash_output),
				signature_output, &sig_len, myrand, NULL );

		if(return_val != 0){
			mbedtls_strerror(return_val,ptr_error_msg,sizeof(error_msg));
			PRINTF("\r\nSignature result: %s \r\n",error_msg);
		}else{
			PRINTF("\r\nSignature result: OK \r\n");
		}
		...
		PRINTF("\r\nSignature:");
		char * my_buf3 =(char *) signature_output;
		for(int i=0;i<sizeof(signature_output);i++){
			PRINTF("%02x,",*my_buf3++);
		}
		PRINTF("\r\n");

		//PRINTF("\r\nECDSA Signed Hash:");
		//char * my_buf4 =(char *) hash_output;
		//for(uint32_t i=0;i<sizeof(hash_output);i++){
		//	PRINTF("%02x,",*my_buf4++);
		//}
		//PRINTF("\r\n");
		...
		PRINTF("\r\nVerifying Signature...\r\n");
		return_val = mbedtls_ecdsa_read_signature( &ecdsa, hash_output, sizeof(hash_output), signature_output, sig_len);
		...
		//char * my_buf5 =(char *) signature_ver_output;
